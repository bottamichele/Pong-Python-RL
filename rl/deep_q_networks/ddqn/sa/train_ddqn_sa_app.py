...
from rl.common.sa.train_sa_app import TrainingSAApp

from .train_ddqn_sa_bot_controller import DDQNTraininingSABotController

class DDQNTrainingSAApp(TrainingSAApp):
    """Application to train a bot that uses DDQN against a bot of Pong."""

    def __init__(self, training_session):
        """Create new application for training of bot controller that uses DDQN.
        
        Parameters
        --------------------
        training_session: DDQNTrainingSASession
            a training session"""

        super().__init__(training_session)

    def _create_controller_1(self):
        self._controller_1 = DDQNTraininingSABotController(self._current_game, self._training_session)
